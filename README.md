...
Борт работает без графического интерфейса (headless). Результат его работы —
видео, телеметрия и индикатор полётного контроллера — виден в полётном окне
станции оператора [MavixDesktop](../MavixDesktop-UI):

| | |
|---|---|
| ![Видео и телеметрия с борта](assets/screenshots/flight.png) | ![Видео с картой](assets/screenshots/flight_map.png) |
| Видеопоток и телеметрия борта, индикатор ФК | Видео борта с картой и положениями стиков |
