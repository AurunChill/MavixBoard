...
| `DRONE_TOKEN не задан` | В `preset.env`/`.env` нет токена дрона | Установить пакет, собранный в системе «Mavix» (токен зашивается автоматически), либо прописать `DRONE_TOKEN` вручную |
